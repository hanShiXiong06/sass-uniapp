...
class Request {
  ...
  constructor() {
    // #ifdef H5
    this.baseUrl =
      import.meta.env.VITE_APP_BASE_URL || `${location.origin}/api/`;
    // #endif
    // #ifndef H5
    ...
    try {
      if (process.env.NODE_ENV == "development") {
        this.config.header[import.meta.env.VITE_REQUEST_HEADER_SITEID_KEY] =
          getSiteId(
            import.meta.env.VITE_SITE_ID || uni.getStorageSync("wap_site_id")
          );
      } else {
        this.config.header[import.meta.env.VITE_REQUEST_HEADER_SITEID_KEY] =
          getSiteId(import.meta.env.VITE_SITE_ID);
      }
  ...
  private requestInterceptors() {
    ...
    try {
      getToken() &&
        (this.config.header[import.meta.env.VITE_REQUEST_HEADER_TOKEN_KEY] =
          getToken());
      this.config.header[import.meta.env.VITE_REQUEST_HEADER_CHANNEL_KEY] =
        getAppChannel();
      if (process.env.NODE_ENV == "development") {
    ...
    return new Promise((resolve, reject) => {
      uni.uploadFile({
        ...
        success: (res) => {
          ...
          if (data.code == 1) {
            this.config.showSuccessMessage &&
              uni.showToast({ title: data.msg, icon: "none" });
            resolve(data);
          } else {
            if (data.code == 0 || data.code == 400) {
              if (this.config.showErrorMessage !== false)
                uni.showToast({ title: data.msg, icon: "none" });
            } else {
  ...
  private request(
    ...
    if (
      ...
      params.method.toUpperCase() == "DELETE"
    ) {
  ...
  private handleRequestFail(res) {
    if (res.errMsg && res.errMsg == "request:ok") {
      if (typeof res.data == "string") {
        ...
        const err =
          (isUrl(this.baseUrl)
            ? this.baseUrl
            : location.origin + this.baseUrl) + t("baseUrlError");
        // #endif
        // #ifndef H5
    ...
    if (res.errMsg && res.errMsg == "request:fail url not in domain list") {
      uni.showToast({
        icon: "none",
        title: this.baseUrl + t("notInDomainList"),
      });
      return;
    }
  }
