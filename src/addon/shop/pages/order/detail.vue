      ...
      <view v-if="!loading" class="pb-20rpx">
        <view v-if="detail.status_name" class="pl-[40rpx] pr-[50rpx] bg-linear pb-[100rpx]">
          ...
          <view class="flex justify-between items-center pt-[30rpx]">
            ...
            <view>
              <image v-if="detail.status == 1" class="w-[110rpx] h-[110rpx]"
                     :src="img('addon/shop/detail/payment.png')"/>
              <image v-if="detail.status == 2" class="w-[110rpx] h-[110rpx]"
                     :src="img('addon/shop/detail/payment.png')"/>
              <image v-if="detail.status == 3" class="w-[110rpx] h-[110rpx]"
                     :src="img('addon/shop/detail/receive.png')"/>
              <image v-if="detail.status == 5" class="w-[110rpx] h-[110rpx]"
                     :src="img('addon/shop/detail/complete.png')"/>
              <image v-if="detail.status == -1" class="w-[110rpx] h-[110rpx]"
                     :src="img('addon/shop/detail/close.png')"/>
            </view>
          </view>
        </view>
        <view class="bg-[#fff] mx-[30rpx] p-[20rpx] mt-[-86rpx] rounded-[16rpx]"
              v-if="detail.delivery_type != 'virtual'">
          <view v-if="detail.delivery_type == 'express'">
            <view class="text-[#303133]">
              <view class="flex items-center">
                <image class="w-[28rpx] h-[32rpx] mr-[10rpx] flex-shrink-0"
                       :src="img('addon/shop/detail/address.png')"/>
                <text class="text-[32rpx] font-500 leading-[38rpx]">{{ detail.taker_address }}</text>
                <text class="text-[22rpx] ml-[10rpx] leading-[26rpx]">
                  {{ detail.taker_full_address.split(detail.taker_address)[0] }}
                </text>
              </view>
          ...
          <view class="order-goods-item flex justify-between flex-wrap mt-[30rpx]"
            ...
            <view class="ml-[20rpx] flex flex-1 flex-col justify-between">
              <view>
                ...
                <view v-if="goodsItem.sku_name">
                  <view class="text-[24rpx] mt-[10rpx] text-[#999] truncate max-w-[450rpx] leading-[28rpx]">
                    {{ goodsItem.sku_name }}
                  </view>
                </view>
              ...
              <view class="flex justify-between items-center leading-[28rpx]">
                <view class="price-font">
                  <text v-if="goodsItem.extend && parseFloat(goodsItem.extend.point) > 0" class="text-[32rpx]">
                    {{ goodsItem.extend.point }}积分
                  </text>
                  <text v-if="parseFloat(goodsItem.price) && goodsItem.extend && parseFloat(goodsItem.extend.point) > 0"
                        class="mx-[4rpx] text-[32rpx]">+
                  </text>
            ...
            <view class="flex justify-end  self-end w-[100%] mt-[10rpx]"
              ...
              <view v-else-if="goodsItem.is_enable_refund == 1"
                    ...
                    @click="applyRefund(goodsItem.order_goods_id)">申请退款
              </view>
            </view>
          </view>
        </view>
          ...
          <view class="bg-[#fff] mx-[30rpx] p-[20rpx] mt-[20rpx] rounded-[16rpx]"
            ...
            <block v-else>
              ...
              <view class="flex items-center justify-center">
                ...
                <text
                    class="text-[#666] text-[24rpx] ml-[10rpx] border-[2rpx] border-solid border-[#666] bg-[#f7f7f7] px-[12rpx] py-[6rpx] rounded"
                    @click="copy(verifyInfo[0].code)">复制
                </text>
              </view>
            </block>
        ...
        <view
            class="flex z-2 justify-end items-center bg-[#fff] fixed left-0 right-0 bottom-0 min-h-[100rpx] px-1 flex-wrap  pb-ios">
          ...
          <view class="flex justify-end mr-[30rpx]">
            ...
            <view
                class="text-[26rpx] leading-[52rpx] px-[23rpx] border-[2rpx] border-solid border-[#999] rounded-full ml-[20rpx] text-[#303133]"
                v-if="detail.status == 1" @click="orderBtnFn('close')">{{ t('orderClose') }}
            </view>
            <view
                class=" text-[26rpx] leading-[52rpx] px-[23rpx] border-[2rpx] border-solid text-[#fff] bg-primary border-primary rounded-full ml-[20rpx]"
                @click="orderBtnFn('pay')" v-if="detail.status == 1">{{ t('topay') }}
            </view>
            <view
                class="text-[26rpx] leading-[52rpx] px-[23rpx] border-[2rpx] border-solid text-[#fff] bg-primary border-primary rounded-full ml-[20rpx]"
                v-if="detail.status == 3" @click="orderBtnFn('finish')">{{ t('orderFinish') }}
            </view>
            <block v-if="detail.status == 5">
              <view v-if="detail.is_evaluate == 1 || (detail.is_evaluate != 1 && evaluateConfig.is_evaluate == 1)"
                    class="text-[26rpx] leading-[52rpx] px-[23rpx] border-[2rpx] border-solid border-[#999] rounded-full ml-[20rpx] !text-[#303133]"
                    @click="orderBtnFn('evaluate')">{{ t('evaluate') }}
              </view>
            </block>
          </view>
        </view>
