		...
		<view class="fixed left-0 top-0 right-0 z-10" v-if="statusLoading">
			<scroll-view :scroll-x="true" class="box-border px-[30rpx] bg-white">
				<view class="flex whitespace-nowrap justify-between py-[24rpx]">
					<view
						:class="['text-[28rpx] leading-[40rpx]', { 'class-select': orderState === item.status.toString() }]"
					...
					<view class="mb-[20rpx] card-template">
						<view @click.stop="toLink(item)">
							<view class="flex justify-between items-center">
								<view class="text-[#626779] text-[24rpx] font-400 leading-[34rpx]">
									...
									<text
										class="text-[#626779] text-[24rpx] font-400 nc-iconfont nc-icon-fuzhiV6xx1 ml-[11rpx]"
										@click.stop="copy(item.order_no)"></text>
								</view>
								<view class="text-[#333] text-[24rpx] leading-[34rpx]"
									:class="{ 'text-primary': item.status == 1, 'text-[#626779]': item.status == 5 || item.status == -1 }">
									{{ item.status_name.name }}</view>
							</view>
							<view class="flex box-border mt-[20rpx]" v-for="(subitem, index) in item.order_goods"
								:key="index">
								...
								<view class="ml-[20rpx] flex flex-1 flex-col box-border">
									<view class="flex justify-between items-baseline">
										<view
											class="max-w-[322rpx] text-[28rpx] leading-[40rpx] font-500 truncate text-[#333]">
											{{ subitem.goods_name }}
										</view>
										<text class="text-[20rpx] text-item leading-[40rpx]">{{ subitem.sub_title
											}}</text>
										<block v-if="item.activity_type == 'exchange'">
											<view class="text-right  ml-[10rpx] leading-[42rpx]"
												v-if="parseFloat(subitem.price)">
												<text class="text-[20rpx] price-font">￥</text>
												<text class="text-[32rpx] font-500 price-font">{{
													parseFloat(subitem.price).toFixed(2).split('.')[0]
												}}</text>
												<text class="text-[22rpx] font-500 price-font">.{{
													parseFloat(subitem.price).toFixed(2).split('.')[1]
												}}</text>
											</view>
										</block>
										<block v-else>
									...
									<view class="flex justify-between items-center text-[#333] mt-[20rpx]">
										<text v-if="subitem.sku_no"
											class="text-[24rpx] truncate mt-[10rpx] text-[#999] leading-[28rpx] max-w-[480rpx]">sn:{{
												subitem.sku_no
											}}</text>
										<view
											class="text-[26rpx] text-[#333] font-400 truncate leading-[36rpx] max-w-[369rpx]">
											{{ subitem.sku_name }}
										</view>
										<text class="text-right text-[26rpx] font-400 w-[90rpx] leading-[36rpx]">x{{
											subitem.num }}</text>
									</view>
									<view class="text-[26rpx] font-400 leading-[36rpx] text-[#333] mt-[10rpx]"
										v-if="item.delivery_type != 'virtual'">
										{{ t('deliveryType') }} ： {{ item.delivery_type_name }}
									</view>
									<view class="text-[26rpx] font-400 leading-[36rpx] text-[#333] mt-[10rpx]" v-else>
										{{ t('createTime') }}
						...
						<view class="flex justify-end items-center mt-[30rpx]">
							<view class="flex items-baseline">
								<view class="text-[22rpx] font-400 text-[#8288A2] leading-[30rpx] mr-[6rpx]"
								...
								<view class="leading-[1]">
									<block v-if="item.activity_type == 'exchange'">
										<text class="text-[32rpx] mr-[2rpx] leading-[40rpx] price-font">{{ item.point
											}}</text>
										...
										<block v-if="parseFloat(item.order_money)">
											...
											<text class="text-[32rpx] font-500 leading-[40rpx] price-font">{{
												parseFloat(item.order_money).toFixed(2).split('.')[0] }}</text>
											<text class="text-[22rpx] font-500 leading-[28rpx] price-font">.{{
												parseFloat(item.order_money).toFixed(2).split('.')[1] }}</text>
											<text class="text-[20rpx] font-500 leading-[28rpx] ml-[2rpx]">{{ t('money')
												}}</text>
										</block>
									</block>
									<block v-else>
						...
						<view class="flex justify-end text-[28rpx] mt-[20rpx] items-center"
							v-if="(item.status == 1) || (item.status == 3) || (item.status == 5 && evaluateConfig.is_evaluate == 1)">
							<view
								class="text-[26rpx] font-400 leading-[56rpx] h-[56rpx] min-w-[150rpx] text-center border-[2rpx] border-solid border-[#8288A2] rounded-full text-[#626779] box-border"
								v-if="item.status == 1" @click.stop="orderBtnFn(item, 'close')">{{ t('orderClose') }}
							</view>
							<view
								class="text-[26rpx] font-400 leading-[56rpx] h-[56rpx] min-w-[150rpx] text-center border-[2rpx] border-solid text-[#fff] border-primary bg-primary rounded-full ml-[20rpx] box-border"
								@click.stop="orderBtnFn(item, 'pay')" v-if="item.status == 1">{{ t('topay') }}</view>
							<view
								class="text-[26rpx] font-400 leading-[56rpx] h-[56rpx] min-w-[150rpx] text-center border-[2rpx] border-solid text-[#fff] border-primary bg-primary rounded-full ml-[20rpx] box-border"
								@click.stop="orderBtnFn(item, 'finish')" v-if="item.status == 3">{{ t('orderFinish') }}
							</view>
							<view
								class="text-[26rpx] font-400 leading-[56rpx] h-[56rpx] min-w-[150rpx] text-center border-[2rpx] border-solid border-[#8288A2] rounded-full ml-[20rpx]  text-[#626779] box-border"
								v-if="item.status == 5 && evaluateConfig.is_evaluate == 1"
								@click.stop="orderBtnFn(item, 'evaluate')">{{
									item.is_evaluate == 1 ? t('selectedEvaluate') : t('evaluate') }}</view>
						</view>
					</view>
				</template>
			</view>
			<view class="mx-[30rpx] mt-[20rpx] rounded-[16rpx] flex items-center justify-center noData"
