<template>
	<view class="keyboard-main" :style="{height:height+'px'}"  style="background: #fff;padding:20rpx">
		<text  style="margin-top:10px" :style="{fontSize:parseInt((fontSize/3*2))+'px'}">{{currency}}</text>
		<view class="keyboard-content" :style="{height:height+'px'}">
			...
			<view class="input" :class="{zIndex:isShow}" :style="{fontSize:size+'px',fontWeight:isBold?'bold':'initial'}" @click.stop="show">
				<span id="text" style="margin-top:-12px">
					<block v-if="isFilter">
						{{filterMoney(money)}}
					</block>
					<block v-if="!isFilter">
					...
					<!-- <view class="line" style="margin-top:-212px" :style="{height:parseInt((height/4*3))+'px','background':btnColor}"></view> -->
				</span>
			</view>
		</view>
...
<style  scoped>
	...
	.keyboard-content .input.zIndex {
		z-index: 999;
	}
	.keyboard-content .input #text {
		display: flex;
		justify-content: flex-start;
		align-items: flex-end;
	}
	.keyboard-content .input .line {
	...
	.keyboard .key-cell.last-child {
		/* margin-right: 0px;*/
		width: 170rpx;
	}
	.keyboard .key-cell .icon {
		width: 50rpx;
		height: 40rpx;
	}
	...
	.keyboard .key-confirm.big {
		right: 20rpx;
		padding: 0 30rpx;
		bottom: 20rpx;
	}
	.keyboard .key-confirm.frist {
		position: absolute;
		top: auto;
		right: 20rpx;
		height: 280rpx;
