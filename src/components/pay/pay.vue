		...
		<view class="flex flex-col h-[75vh] popup-common" v-if="payInfo" @touchmove.prevent.stop>
			...
			<scroll-view scroll-y="true" class="flex-1 pt-[20rpx]">
				...
				<view class="mx-[var(--popup-sidebar-m)] px-[30rpx] bg-white rounded-[20rpx] bg-[var(--temp-bg)]">
					...
					<view class="py-[30rpx] text-center text-[24rpx] text-gray-subtitle" v-else>{{
						t('pay.notHavePayType') }}</view>
				</view>
			</scroll-view>
			<view class="btn-wrap">
				<button class="primary-btn-bg btn" hover-class="none" :loading="loading"
					@click="confirmPay">{{ t('pay.confirmPay') }}</button>
			</view>
		</view>
	</u-popup>
</template>
